...
function [joint_distribution,delay_distribution]=joint_distribution_TE(binned_inputs,input_code,binned_outputs,binned_delayed_outputs,output_code,wordsize)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%% Map delayed outputs to codes %%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if strcmp(output_code,'spatial')
        all_words=double(dec2bin(0:2^n_trial_out-1)' - '0'); % All possible input spiking patterns
        i_outputs=zeros(n_rep,t_len);
        for rep_ind=1:n_rep
            [~,j_outputs]=ismember(squeeze(binned_delayed_outputs(:,rep_ind,:))',all_words','rows'); % Match output patterns
            i_outputs(rep_ind,:)=j_outputs;
        end
        edge_vec=linspace(0.5,2^n_trial_out+0.5,2^n_trial_out+1);
        delay_distribution=histcounts(i_outputs(:),edge_vec)';
        cd_delays=reshape(i_outputs,[1,n_rep,t_len]);
    elseif strcmp(output_code,'rate_ensemble')
        binned_outputs=sum(binned_outputs,1); % Take ensemble rates
        max_output_rate=max(binned_delayed_outputs(:));
        i_outputs=zeros(n_rep,t_len);
        for rep_ind=1:n_rep
            i_outputs(rep_ind,:)=binned_delayed_outputs(1,rep_ind,:)+1;
        end
        edge_vec=linspace(-0.5,max_output_rate+0.5,max_output_rate+2);
        delay_distribution=histcounts(binned_delayed_outputs(:),edge_vec);
        cd_delays=reshape(i_outputs,[1,n_rep,t_len]);
    elseif strcmp(output_code,'rate_local')
        max_output_rate=max(binned_delayed_outputs(:));
        i_outputs=zeros(n_trial_out,n_rep,t_len);
        for trial_ind=1:n_trial_out
            for rep_ind=1:n_rep
                i_outputs(trial_ind,rep_ind,:)=binned_delayed_outputs(trial_ind,rep_ind,:)+1;
            end
        end
        edge_vec=linspace(-0.5,max_output_rate+0.5,max_output_rate+2);
        delay_distribution=histcounts(binned_delayed_outputs(:),edge_vec);
        cd_delays=reshape(i_outputs,[n_trial_out,n_rep,t_len]);
    elseif strcmp(output_code,'temporal')
        all_words=double(dec2bin(0:2^wordsize-1)' - '0'); % All possible input spiking patterns
        i_outputs=zeros(n_trial_out,n_rep,t_len);
        for trial_ind=1:n_trial_out
            for rep_ind=1:n_rep
                ordered_outputs=reshape(binned_delayed_outputs(trial_ind,rep_ind,:),[wordsize,t_len]);
                [~,j_outputs]=ismember(ordered_outputs',all_words','rows'); % Match input patterns
                i_outputs(trial_ind,rep_ind,:)=j_outputs;
            end
        end
    ...
    joint_distribution=zeros(length(input_distribution),length(output_distribution),length(delay_distribution));
    if ex_in==ex_out % Matched inputs and outputs
        for trial_ind=1:ex_in
            for rep_ind=1:n_rep
               joint_distribution=joint_distribution+accumarray([squeeze(cd_inputs(trial_ind,1,:)) , squeeze(cd_outputs(trial_ind,rep_ind,:)) , squeeze(cd_delays(trial_ind,rep_ind,:))],1,[length(input_distribution),length(output_distribution),length(delay_distribution)]);
            end
        end
    elseif ex_in==1 % Input ensemble code
        for trial_ind=1:ex_out
            for rep_ind=1:n_rep
                joint_distribution=joint_distribution+accumarray([squeeze(cd_inputs(1,1,:)) , squeeze(cd_outputs(trial_ind,rep_ind,:)) , squeeze(cd_delays(trial_ind,rep_ind,:))],1,[length(input_distribution),length(output_distribution),length(delay_distribution)]);
            end
        end
    elseif ex_out==1 % Output ensemble code
        for trial_ind=1:ex_in
            for rep_ind=1:n_rep
                joint_distribution=joint_distribution+accumarray([squeeze(cd_inputs(trial_ind,1,:)) , squeeze(cd_outputs(1,rep_ind,:)) , squeeze(cd_delays(1,rep_ind,:))],1,[length(input_distribution),length(output_distribution),length(delay_distribution)]);
            end
        end
        ...
        for trial_ind_in=1:ex_in
            for trial_ind_out=1:ex_out
                for rep_ind=1:n_rep
                    joint_distribution=joint_distribution+accumarray([squeeze(cd_inputs(trial_ind_in,1,:)) , squeeze(cd_outputs(trial_ind_out,rep_ind,:)) , squeeze(cd_delays(trial_ind,rep_ind,:))],1,[length(input_distribution),length(output_distribution),length(delay_distribution)]);
                end
            end
        end
    end
